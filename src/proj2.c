...
void clean_up(int stage, int busstop_count){
    ...
    if (stage >= 5) munmap(current_busstop_m,sizeof(sem_t));
    if (stage >= 6) munmap(waitingskier_count,sizeof(int));
    if (stage >= 7) munmap(waitingskier_count_m,sizeof(sem_t));
    if (stage >= 8) munmap(busstop_waiting_count,sizeof(int)*busstop_count);
    if (stage >= 9) munmap(busstop_waiting_count_m,sizeof(sem_t));
    if (stage >= 10) munmap(current_buscapacity,sizeof(int));
    if (stage >= 11) munmap(current_buscapacity_m,sizeof(sem_t));
    if (stage >= 12) munmap(busstops_ms,sizeof(sem_t)*(busstop_count+1));
    if (stage >= 13) munmap(skiers_boarding_m,sizeof(sem_t));
    if (stage >= 14) munmap(busatstop_m ,sizeof(sem_t));
    if (stage >= 15) sem_destroy(line_count_m);
    if (stage >= 16) sem_destroy(skier_id_m);
    if (stage >= 17) sem_destroy(current_busstop_m);
    if (stage >= 18) sem_destroy(waitingskier_count_m);
    if (stage >= 19) sem_destroy(busstop_waiting_count_m);
    if (stage >= 20) sem_destroy(current_buscapacity_m);
    if (stage >= 21){
        for (int i = 0; i < (busstop_count+1); i++) {
                sem_destroy(&busstops_ms[i]);
            }
    }
    if (stage >= 21) sem_destroy(skiers_boarding_m);
    if (stage >= 22) sem_destroy(busatstop_m);
    ...
    while(true){
        if (*current_busstop <= busstop_count ){
            ...
            if (busstop_waiting_count[*current_busstop - 1] > 0 && *current_buscapacity > 0){
                ...
                sem_post(&busstops_ms[*current_busstop - 1]);
                //waiting for skiers to board
                sem_wait(skiers_boarding_m);
                //leaving stop
            ...
            if (*current_buscapacity < bus_capacity){
                ...
                sem_post(&busstops_ms[busstop_count]);
...
void handlingSkier(int * line_count, int * skier_id, int random_busstop, int *waitingskier_count, int busstop_count, int *busstop_waiting_count, int *current_buscapacity, int bus_capacity){
    ...
    while (true){
        //waiting for bus to arrive at skiers stop
		sem_wait(&busstops_ms[random_busstop - 1]);
    	//skier can try to board
		sem_wait(current_buscapacity_m);
		if (*current_buscapacity > 0){
			//skier can board
            sem_wait(busstop_waiting_count_m);
            sem_wait(waitingskier_count_m);
			sem_wait(line_count_m);
            ...
            if (*current_buscapacity == 0 || busstop_waiting_count[(random_busstop) - 1] == 0){
                //last skier boarding allowing bus to leave
                sem_post(skiers_boarding_m);
            }
            ...
            sem_post(current_buscapacity_m);
			sem_post(line_count_m);
			sem_post(&busstops_ms[random_busstop - 1]);
    ...
    if (*current_buscapacity == bus_capacity){
        //last skier aboarding allowing bus to leave final
        sem_post(skiers_boarding_m);
    }
...
int main(int argc, char *argv[]) {
    ...
    busstops_ms = mmap(NULL, sizeof(sem_t)*(busstop_count+1), PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, 0, 0);
    if (busstops_ms == MAP_FAILED) {
        clean_up(12, busstop_count);
        fprintf(stderr, "Error: mmap() failed for busstops_ms.\n");
        exit(1);
    }
    skiers_boarding_m = mmap(NULL, sizeof(sem_t), PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, 0, 0);
    if (skiers_boarding_m == MAP_FAILED) {
        clean_up(13, busstop_count);
        fprintf(stderr, "Error: mmap() failed for skiers_boarding_m.\n");
        exit(1);
    }
    busatstop_m = mmap(NULL, sizeof(sem_t), PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, 0, 0);
    if (busatstop_m == MAP_FAILED) {
    ...
    if (sem_init(line_count_m, 1, 1) != 0) {
        fprintf(stderr, "Error: Semaphore line_count_m initialization failed.\n");
        clean_up(15, busstop_count);
	    exit(1);
    }
    if (sem_init(skier_id_m, 1, 1) != 0) {
        fprintf(stderr, "Error: Semaphore skier_id_m initialization failed.\n");
        clean_up(16, busstop_count);
	    exit(1);
    }
    if (sem_init(current_busstop_m, 1, 1) != 0) {
        fprintf(stderr, "Error: Semaphore current_busstop_m initialization failed.\n");
        clean_up(17, busstop_count);
	    exit(1);
    }
    if (sem_init(waitingskier_count_m, 1, 1) != 0) {
        fprintf(stderr, "Error: Semaphore waitingskier_count_m initialization failed.\n");
        clean_up(18, busstop_count);
	    exit(1);
    }
    if (sem_init(busstop_waiting_count_m, 1, 1) != 0) {
        fprintf(stderr, "Error: Semaphore busstop_waiting_count_m initialization failed.\n");
        clean_up(19, busstop_count);
	    exit(1);
    }
    if (sem_init(current_buscapacity_m, 1, 1) != 0) {
        fprintf(stderr, "Error: Semaphore current_buscapacity_m initialization failed.\n");
    ...
    if (sem_init(skiers_boarding_m, 1, 0) != 0) {
        fprintf(stderr, "Error: Semaphore skiers_boarding_m initialization failed.\n");
        clean_up(21, busstop_count);
	    exit(1);
    }
    if (sem_init(busatstop_m, 1, 1) != 0) {
        fprintf(stderr, "Error: Semaphore busatstop_m initialization failed.\n");
    ...
    if (process_bus < 0){
        ...
        handlingBus(line_count, current_busstop, busstop_count, travel_time, waitingskier_count, busstop_waiting_count, current_buscapacity, bus_capacity);
        exit(0);
    }
    ...
    for (int i = 0; i < skier_count; i++){
        ...
        if (process_skier < 0){
            ...
            handlingSkier(line_count, skier_id, random_busstop, waitingskier_count, busstop_count, busstop_waiting_count, current_buscapacity, bus_capacity);
            sem_post(skier_id_m);
            exit(0);
        }
    }
